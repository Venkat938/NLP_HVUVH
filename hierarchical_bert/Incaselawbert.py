...
train_dataset = pd.read_csv('train_text.csv')
test_dataset = pd.read_csv('test_text.csv')
dev_dataset = pd.read_csv('val_text.csv')

train_dataset.drop(['Unnamed: 0'], axis = 1, inplace = True)
test_dataset.drop(['Unnamed: 0'], axis = 1, inplace = True)
dev_dataset.drop(['Unnamed: 0'], axis = 1, inplace = True)
...
def process(text):
    ...
    if(len(sentances) < 16):
        sentances.extend(['']*(16-len(sentances)))
    return sentances[:16]
...
if __name__ == "__main__":
    ...
    bert = AutoModel.from_pretrained('law-ai/InCaseLawBERT')
    model = HierarchicalBert(encoder=bert, max_segments= 32, max_segment_length=64)
